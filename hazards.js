...
function spawnBounceCircle() {
  ...
  const colors = ["#d42426", "#ffd700", "#2f5a28", "#1e90ff"];
...
export function drawHazards(ctx) {
  ...
  for (const s of saws) {
    ...
    for (let i = 0; i < s.trail.length; i++) {
      ...
      if (alpha <= 0) continue;
      // Icy blue trail
      ctx.strokeStyle = `rgba(135,206,235,${0.4 * alpha})`;
  ...
  // Draw saws (Snowflakes)
  for (const s of saws) {
    ctx.save();
    ctx.translate(s.x, s.y);
    ctx.rotate(s.angle);

    // Snowflake color: White/LightCyan with dark outline
    ...
    for (let i = 0; i < branches; i++) {
      ...
      ctx.moveTo(0, 0);
      ctx.lineTo(0, -s.r);
      // little V shape at end
      ctx.moveTo(-s.r * 0.3, -s.r * 0.6);
      ctx.lineTo(0, -s.r * 0.85);
      ctx.lineTo(s.r * 0.3, -s.r * 0.6);
    }
    ctx.stroke();
  ...
  for (const ls of largeSaws) {
    ...
    if (now >= warningWindowStart && now < ls.explodeTime) {
      ...
      for (const dir of ls.directions || []) {
        ...
        ctx.strokeStyle = `rgba(178,34,34,${alphaWarn * 0.4})`; // FireBrick red warning
        ctx.beginPath();
        ctx.moveTo(sx, sy);
        ctx.lineTo(ex, ey);
        ctx.stroke();
        ...
        ctx.lineWidth = 2.1;
        ctx.strokeStyle = `rgba(178,34,34,${alphaWarn})`;
        ctx.beginPath();
        ctx.moveTo(sx, sy);
        ctx.lineTo(ex, ey);
        ctx.stroke();
    ...
    ctx.fillStyle = "#f0f8ff"; // Match background
    ctx.fill();
    ctx.stroke();

    // Berries (blink red)
    ctx.fillStyle = `rgb(${200 + 55 * blinkAmount}, 0, 0)`;
    const berryCount = 8;
    const berryDist = radius * 0.8;
    for (let i=0; i<berryCount; i++) {
        const a = (i / berryCount) * Math.PI * 2 + activeT;
        const bx = Math.cos(a) * berryDist;
        const by = Math.sin(a) * berryDist;
        ctx.beginPath();
  ...
  // Draw bouncing circle hazards (Ornaments)
  for (const bc of bounceCircles) {
    const { x, y, spawnTime, appearTime, disappearTime, maxRadius, color } = bc;
    ctx.save();
    ctx.translate(x, y);
    ...
    if (now < appearTime) {
      ...
      if (radius > 0) {
        ...
        // Cap
        ctx.fillStyle = "#c0c0c0"; // Silver cap
        ctx.fillRect(-radius*0.2, -radius, radius*0.4, radius*0.3);

        const ringRadius = radius * (0.55 + 0.15 * (1 - easeOut));
        const ringAlpha = 0.35 + 0.35 * easeOut;
        ctx.globalAlpha = ringAlpha;
        ctx.strokeStyle = color || "#d42426";
        ctx.lineWidth = 3;
        ctx.beginPath();
  ...
  for (const lw of largeWarnings) {
    ...
    if (!lw.fired) {
      if (lw.isBarrage) {
        ...
        ctx.strokeStyle = `rgba(34,139,34,${alpha})`; // Forest Green
        ctx.lineWidth = widthLine;
        ctx.beginPath();
        ctx.moveTo(startX, startY);
        ctx.lineTo(endX, endY);
        ...
        // Green warning
        if (stage < 1 / 3) {
          ctx.strokeStyle = "rgba(34,139,34,0.18)";
          ctx.lineWidth = 4 * pulse;
        } else if (stage < 2 / 3) {
          ctx.strokeStyle = "rgba(34,139,34,0.3)";
          ctx.lineWidth = 7 * pulse;
        } else {
          ctx.strokeStyle = "rgba(34,139,34,0.45)";
      ...
      // Green Laser Glow
      ctx.strokeStyle = `rgba(34,139,34,${outerAlpha})`;
      ...
      ctx.strokeStyle = `rgba(0,100,0,${coreAlpha})`; // Dark Green Core
