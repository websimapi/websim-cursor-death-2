...
let sfxVolume = 1.0;
let musicVolume = 0.1;
// Use new holiday asset as default music
let musicUrl = "/christmas_loop.mp3";
...
export function setCustomMusicUrl(url) {
  const newUrl = url && typeof url === "string" ? url : "/christmas_loop.mp3";
  musicUrl = newUrl;
  if (!bgmAudio) {
    bgmAudio = new Audio(musicUrl);
    bgmAudio.loop = true;
...
export function playClickBeginHellSound() {
  ...
  // Sleigh bell jingle-ish
  playTone({ freq: 880, duration: 0.1, type: "sine", volume: 0.04, attack: 0.01, decay: 0.09, startTime: now });
  playTone({ freq: 1100, duration: 0.1, type: "sine", volume: 0.04, attack: 0.01, decay: 0.09, startTime: now + 0.05 });
  playTone({ freq: 1320, duration: 0.15, type: "sine", volume: 0.03, attack: 0.01, decay: 0.14, startTime: now + 0.1 });
}
