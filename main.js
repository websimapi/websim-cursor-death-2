...
const DEFAULT_CURSOR_SRC = "/cursor_christmas.png";
...
function resetSettingsToDefaults() {
  ...
  setCustomCursorImage(null);           // uses /cursor_christmas.png internally
  setCustomMusicUrl(null);             // uses /christmas_loop.mp3 internally
...
function spawnExplosion(x, y) {
  ...
  const now = performance.now() / 1000;
  const count = 45;
  const colors = ["#d42426", "#2f5a28", "#ffd700", "#ffffff"]; // Red, Green, Gold, White
  explosionParticles = [];
...
function drawExplosion() {
  ...
  for (const p of explosionParticles) {
    ...
    if (alpha <= 0) continue;
    ctx.fillStyle = p.color || `rgba(0,0,0,${alpha})`;
    // fading out
